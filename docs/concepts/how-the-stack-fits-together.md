...
The first stack is a local offline chat service. It is intentionally narrow so the reader can build something real before exploring larger deployment patterns.
...
| Operations | [Open WebUI operations](../operations/open-webui-ops.md) | How do you maintain the service after setup? |
...
```mermaid
flowchart TD
  Q[Quickstart] --> G[Offline chat service]
  G --> H[Mac Mini 24 GB]
  G --> E[Development environment]
  G --> O[Ollama]
  G --> M[Qwen3.5-9B]
  G --> W[Open WebUI]
  G --> R[Open WebUI operations]
```
